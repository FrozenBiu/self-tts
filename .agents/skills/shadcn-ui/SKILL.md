...
**A. Direct Installation (Recommended with `pnpm dlx`)**
```bash
pnpm dlx shadcn@latest add [component-name]

# Ví dụ cài đặt Sonner Toaster bắt buộc:
pnpm dlx shadcn@latest add sonner
```

This command:
- Downloads the component source code (adapting to your config: Radix vs Base UI)
- Installs required dependencies via `pnpm`
...
3. Install peer dependencies manually with `pnpm add`
