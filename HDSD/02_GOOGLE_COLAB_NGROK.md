...
3. Nếu chưa có, bạn bấm nút **+ New Domain** (hoặc *Create Domain*). Ngrok sẽ cấp cho bạn một domain tĩnh miễn phí dạng:
   `xxxx-xxxx-xxxx.ngrok-free.app` (hoặc `.ngrok-free.dev`).
   *(Ví dụ: `smarty-kenia-lawlessly.ngrok-free.dev`)*.
...
2. Bấm vào tab **Upload (Tải lên)** ➔ Chọn file [`notebooks/OmniVoice_Colab_T4.ipynb`](file:///d:/Coding/VSCode/self-tts/notebooks/OmniVoice_Colab_T4.ipynb) từ thư mục dự án trong máy bạn.

### Bước 3.2: Bật GPU T4
1. Trên thanh menu trên cùng của Colab, chọn: **Runtime (Thời gian chạy)** ➔ **Change runtime type (Thay đổi loại thời gian chạy)**.
2. Tại mục *Hardware accelerator (Phần cứng tăng tốc)*, chọn **T4 GPU**.
3. Bấm nút **Save (Lưu)**.
...
*(Lưu ý: Không nhập ở bảng Secrets 🔑 bên cột trái mà nhập trực tiếp vào ô form ở giữa màn hình)*.
